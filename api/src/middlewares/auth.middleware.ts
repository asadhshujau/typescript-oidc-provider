...
export const authenticate: Middleware = async (ctx: Context, next) => {
    ...
    const url = `${process.env.OIDC_ISSUER_URL}/token/introspection`
...
export const authorize = (...scopes: string[]): Middleware => async (ctx, next) => {
    if (
        ctx.state.session &&
